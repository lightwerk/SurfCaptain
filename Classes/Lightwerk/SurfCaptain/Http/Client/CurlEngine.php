...
class CurlEngine extends \TYPO3\Flow\Http\Client\CurlEngine
{
    ...
    public function sendRequest(Request $request)
    {
        if (!extension_loaded('curl')) {
            throw new \TYPO3\Flow\Http\Exception(
        ...
        if ($curlResult === false) {
            throw new CurlEngineException(
                'cURL reported error code ' . curl_errno($curlHandle)
                . ' with message "' . curl_error($curlHandle) . '". Last requested URL was "'
